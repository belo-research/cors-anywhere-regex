...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    function isRegexWhitelisted(url) {
      return corsAnywhere.originWhitelist.some((whiteListedRegex) =>
        new RegExp(`(${whiteListedRegex})`).test(url)
      );
    }
    ...
    if (corsAnywhere.originWhitelist.length && isRegexWhitelisted(origin)) {
      res.writeHead(403, 'Forbidden', cors_headers);
